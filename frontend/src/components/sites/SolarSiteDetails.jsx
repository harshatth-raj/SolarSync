...
export default function SolarSiteDetails() {
  ...
  useEffect(() => {
    ...
    if (r1 && r1.then) r1.then((res) => setSite(res.data)).catch(() => {});
    const r2 = axios.get('/api/sites/1/panels');
    if (r2 && r2.then) r2.then((res) => {
      const data = res.data;
  ...
  return (
    <div>
      <h2>{site.siteName}</h2>
      <button onClick={handleSimulate}>Simulate Generation</button>
      {panels.map((p) => (
        <div key={p.id}>
